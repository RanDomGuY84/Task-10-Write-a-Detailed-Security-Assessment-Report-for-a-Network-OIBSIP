...
Objectives of the Nmap Scan
    ...
    Identify open ports on devices connected to the Multillidea network.
    Determine active services and versions running on those ports.
    Fingerprint operating systems and detect possible vulnerabilities.
    Highlight security concerns related to exposed services.
    Offer recommendations to strengthen network security.

Methodology

The following Nmap techniques were used during the scan:

    Ping Scan (-sn) – to identify live hosts on the network.
    Port Scan (-sS, -sU) – to detect open TCP and UDP ports using SYN and UDP scanning techniques.
    Service Version Detection (-sV) – to determine service type and version.
    OS Fingerprinting (-O) – to estimate the operating system of each host.
    Vulnerability Scripts (--script vuln) – optional scan for common vulnerabilities using Nmap Scripting Engine (NSE).
...
Key Findings
    ...
    Overall network is functional, with expected services responding.
    Multiple hosts expose services unnecessarily, increasing the attack surface.
    Several security risks were identified, particularly related to unencrypted protocols and outdated service versions.
    OS detection shows a mixture of Windows and Linux devices, with varying patch levels.
    No immediate evidence of intrusion, but the exposed services could allow future attacks if unaddressed.
